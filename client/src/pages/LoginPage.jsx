...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (!FACEBOOK_APP_ID || FACEBOOK_APP_ID === 'TU_FACEBOOK_APP_ID_AQUI') return;
  ...
  const handleFacebookLogin = useCallback(async () => {
    ...
    try {
      if (!window.FB) {
        setError('El SDK de Facebook no se ha cargado. Verifica tu FACEBOOK_APP_ID.');
        setSocialLoading('');
        return;
      }
        ...
        async (response) => {
          if (response.authResponse) {
            try {
              ...
              toast.success('¡Bienvenido! Sesión iniciada con Facebook');
              navigate('/');
            } catch (err) {
  ...
  }, [loginWithFacebook, navigate, toast]);

  const isSocialConfigured = 
    (GOOGLE_CLIENT_ID && GOOGLE_CLIENT_ID !== 'TU_GOOGLE_CLIENT_ID_AQUI') ||
    (FACEBOOK_APP_ID && FACEBOOK_APP_ID !== 'TU_FACEBOOK_APP_ID_AQUI');
          ...
          <div className="login-social-section">
            ...
            {!isSocialConfigured && (
              <p className="login-social-hint">
                Configura las credenciales de Google y Facebook en el archivo .env para habilitar el login social
              </p>
            )}
          </div>
        </div>
